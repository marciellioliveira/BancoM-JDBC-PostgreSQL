...
public class ContasRowMapper implements RowMapper<Conta> {
	 ...
	 @Override
	    public Conta mapRow(ResultSet rs, int rowNum) throws SQLException {
	        ...
	        // Cria a conta específica baseada no tipo
	        if (tipoConta == TipoConta.CORRENTE) {
	            BigDecimal taxaManutencao = rs.getBigDecimal("taxa_manutencao_mensal");
	            ContaCorrente cc = new ContaCorrente();
	            cc.setTaxaManutencaoMensal(taxaManutencao);
	            return setCommonFields(cc, id, clienteId, tipoConta, categoriaConta, saldo, numeroConta, pixAleatorio, status);
	            ...
	            return setCommonFields(cp, id, clienteId, tipoConta, categoriaConta, saldo, numeroConta, pixAleatorio, status);
	        } else {
	            return setCommonFields(new Conta(), id, clienteId, tipoConta, categoriaConta, saldo, numeroConta, pixAleatorio, status);
	        }
	    }
	    
	    private Conta setCommonFields(Conta conta, Long id, Long clienteId, TipoConta tipoConta,
	                                CategoriaConta categoriaConta, BigDecimal saldo,
	                                String numeroConta, String pixAleatorio, boolean status) {
	        conta.setId(id);
	        
	        // Cria um cliente mínimo apenas com o ID
