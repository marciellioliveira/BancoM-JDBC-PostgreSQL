...
public class UserClienteService implements UserDetailsService {
	...
	public UserClienteService(UserDao userDao, ClienteDao clienteDao) {
	...
	public User save(UserCreateDTO dto) {
		...
		try {
			...
			// Se ainda assim der erro de duplicidade, trata com uma exceção customizada
			// mais clara
	...
	public User getUserById(Long id, Authentication authentication) throws ClienteEncontradoException {
		...
		if ("ROLE_ADMIN".equals(role)) {
			...
			user = userDao.findById(id).orElseThrow(() -> new ClienteNaoEncontradoException("Usuário não encontrado."));
		...
		if (user.getCliente() != null) {
			...
			clienteComContas.getContas().forEach(conta -> System.out
					.println("Conta ID " + conta.getId() + ", clienteNome: " + conta.getClienteNome()));
		}
		...
		for (User user : users) {
			if (user.getCliente() != null) {
				...
				clienteCompleto.getContas().forEach(conta -> {
					...
					// System.out.println("Conta ID " + conta.getId() + ", clienteNome: " +
					// conta.getClienteNome());

					// iterando sobre as transferências de cada conta
					conta.getTransferencias().forEach(transferencia -> {
						System.out.println("  Transferência Enviada ID: " + transferencia.getId() + ", Valor: "
	...
	public User update(Long id, UserCreateDTO dto, Authentication authentication) throws ClienteEncontradoException {
		...
		User user = userDao.findById(id)
				.orElseThrow(() -> new ClienteNaoEncontradoException("Usuário não encontrado para atualização"));
