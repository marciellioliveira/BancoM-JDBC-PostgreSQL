...
public class UserContaService {
	...
	public UserContaService(ContaDao contaDao, ExchangeRateService exchangeRateService, ClienteDao clienteDao,
			UserDao userDao, TransferenciaDao transferenciaDao, GerarNumeros gerarNumero) {
	...
	public Conta save(ContaCreateDTO dto, Authentication authentication) {
		...
		if ("ROLE_BASIC".equals(role) && !dto.idUsuario().equals(loggedInUser.getId().longValue())) {
			throw new ClienteNaoEncontradoException(
					"Usuário BASIC não tem permissão para criar conta para outro usuário.");
		}
		...
		User userAssociado = cliente.getUser(); // Aqui é parar verificar se o usuario associado ao cliente está
												// desativado
	...
	public Conta getContasById(Long id, Authentication authentication) {
		...
		if ("ROLE_ADMIN".equals(role)) {
			...
			return contaDao.findById(id).orElseThrow(() -> new ContaNaoEncontradaException("Conta não encontrada."));
		} else if ("ROLE_BASIC".equals(role)) {
			// Basic só pode acessar a conta dele mesmo
			return contaDao.findByIdAndUsername(id, username).orElseThrow(() -> new ContaNaoEncontradaException(
	...
	public boolean delete(Long idConta, ContaUpdateDTO dto, Authentication authentication) {
		...
		if ("ROLE_ADMIN".equals(role)) {
			...
			// Verificar se o ADMIN está tentando deletar SUA PRÓPRIA conta
			User loggedInUser = userDao.findByUsername(username)
					.orElseThrow(() -> new ClienteNaoEncontradoException("Usuário logado não encontrado."));

			if (contaExistente.getCliente().getUser().getId().equals(loggedInUser.getId())) {
				throw new ContaExibirSaldoErroException("Administradores não podem deletar a própria conta.");
			}

			// ADMIN pode deletar contas de outros usuários
			logger.info("ADMIN deletando conta ID: {}", contaExistente.getId());

		} else if ("ROLE_BASIC".equals(role)) {
			User loggedInUser = userDao.findByUsername(username)
					.orElseThrow(() -> new ClienteNaoEncontradoException("Usuário logado não encontrado."));

			// BASIC só pode deletar suas próprias contas
			...
			contaExistente = contaDao.findByIdAndUsername(idConta, username)
					.orElseThrow(() -> new ContaNaoEncontradaException(
							"Conta não encontrada ou você não tem permissão para deletá-la."));
			logger.info("BASIC deletando conta ID: {}", contaExistente.getId());
		} else {
		...
		if (contaExistente.getSaldoConta().compareTo(BigDecimal.ZERO) > 0) {
			throw new ContaExibirSaldoErroException("A conta possui um saldo de R$ " + contaExistente.getSaldoConta()
					+ ". Faça o saque antes de remover a conta.");
		} else if (contaExistente.getSaldoConta().compareTo(BigDecimal.ZERO) < 0) {
			throw new ContaExibirSaldoErroException(
					"A conta está com saldo negativo. Regularize antes de remover a conta.");
		}
	...
	public boolean transferirTED(Long idContaReceber, UserContaTedDTO dto, Authentication authentication) {
		...
		if (isAdmin) {
			...
			if (!contaOrigem.getCliente().getId().equals(dto.idUsuario())) {
				System.out.println("Falha: Conta origem Cliente ID " + contaOrigem.getCliente().getId()
	...
	public boolean transferirDEPOSITO(Long idContaReceber, UserContaDepositoDTO dto, Authentication authentication) {
		...
		if (isAdmin) {
			// ADMIN: Verifica se a conta destino pertence ao idUsuario do DTO (Admin pode
			// depositar em qualquer conta)
			if (dto.idUsuario() == null) {
	...
	public boolean transferirSAQUE(Long idContaReceber, UserContaSaqueDTO dto, Authentication authentication) {
		...
		if (isAdmin) {
			// ADMIN: Verifica se a conta de saque pertence ao idUsuario do DTO (Admin pode
