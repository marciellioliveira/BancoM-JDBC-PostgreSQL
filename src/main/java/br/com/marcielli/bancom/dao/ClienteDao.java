...
//							String tipoTransferenciaStr = rs.getString("tipo_transferencia");
//
//							// Verificando quem enviou
//							if (clienteId.equals(idClienteOrigem)) {
//								logger.info("Iniciando processamento da transferência enviada: Transferência ID: {}",
//										transferenciaId);
//								Transferencia transferencia = new Transferencia();
...
//								try {
//									transferencia.setTipoTransferencia(tipoTransferenciaStr != null
//											? TipoTransferencia.valueOf(tipoTransferenciaStr)
//											: null);
//								} catch (IllegalArgumentException e) {
//									logger.warn("TipoTransferencia inválido: {}. Definindo como null.",
//											tipoTransferenciaStr);
//									transferencia.setTipoTransferencia(null);
//								}
//
//								transferencia.setCodigoOperacao(rs.getString("codigo_operacao"));
...
//								String tipoCartaoStr = rs.getString("tipo_cartao");
//								try {
//									transferencia.setTipoCartao(
//											tipoCartaoStr != null ? TipoCartao.valueOf(tipoCartaoStr) : null);
//								} catch (IllegalArgumentException e) {
//									logger.warn("TipoCartao inválido: {}. Definindo como null.", tipoCartaoStr);
//									transferencia.setTipoCartao(null);
//								}
//
//								// adicionando a lista principal transferenciasEnviadas (todas as transferencias
//								// que forem feitas)
//								conta.getTransferencias().add(transferencia);
//								logger.debug("Transferência enviada adicionada à conta {}: Transferência ID {}",
//										conta.getId(), transferencia.getId());
//
//								// se a transferencia foi feita por cartão de crédito, adiciona também a lista
//								// transferenciasCredito
//								if (idCartaoTransferencia != null && idCartaoTransferencia > 0
//										&& "CARTAO_CREDITO".equals(tipoTransferenciaStr)) {
//
//									conta.getCartoes().stream()
//											.filter(cartao -> cartao.getId().equals(idCartaoTransferencia)).findFirst()
//											.ifPresent(cartao -> {
//												if (cartao instanceof CartaoCredito) {
//													((CartaoCredito) cartao).getTransferenciasCredito()
//															.add(transferencia);
//													logger.debug(
//															"Transferência de crédito adicionada ao cartão {}: Transferência ID {}",
