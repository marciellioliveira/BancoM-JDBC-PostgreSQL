-- PROCEDURE: public.criar_usuario_completo_v2(text, text, boolean, text, bigint, boolean, text, text, text, text, text, text, text, text)

-- DROP PROCEDURE IF EXISTS public.criar_usuario_completo_v2(text, text, boolean, text, bigint, boolean, text, text, text, text, text, text, text, text);
...
BEGIN
    ...
    IF v_role_id IS NULL THEN
        ...
        INSERT INTO roles (name) VALUES (p_role_name) RETURNING id INTO v_role_id;
    END IF;

    -- Vincula role ao usuário
    PERFORM vincular_role_v1(v_user_id, p_role_name);
...
END;
$BODY$;
ALTER PROCEDURE public.criar_usuario_completo_v2(text, text, boolean, text, bigint, boolean, text, text, text, text, text, text, text, text)
    OWNER TO postgres;
